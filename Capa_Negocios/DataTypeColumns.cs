...
namespace Capa_Negocios
{
    public class DataTypeColumns
    {
        ...
        public bool canBeDate (DataColumn dC)
        {
            ...
            {
                if (!Regex.IsMatch((String)row[dC], @"(?<!\d)(?:(?:(?:1[6-9]|[2-9]\d)?\d{2})(?:(?:(?:0[13578]|1[02])31)|(?:(?:0[1,3-9]|1[0-2])(?:29|30)))|(?:(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00)))0229)|(?:(?:1[6-9]|[2-9]\d)?\d{2})(?:(?:0?[1-9])|(?:1[0-2]))(?:0?[1-9]|1\d|2[0-8]))(?!\d)"))
                {//Obviamente expresion regular de internet :v
                    cantBe = false;
                }
            }
            return cantBe;
        ...
        public bool canBeInt (DataColumn dC)
        {
            ...
            { //Recorre cada registro.
                ...
                { //Valida que cada char sea numerico
                    if (!Char.IsNumber(c))
                    {
                        isNumber = false;
                    }
                ...
                if ((num < -2147483648 || num > 2147483648) && isNumber)
                { //Si el dato es completamente numerico verifico el tamaño INT (Ver tabla de tipos de datos en la pagina de Microsoft)
                    cantBe = false;
                }
            }
            return cantBe;
